...
* asynchronous API to put job in the background the LogVSOutputHelper
...
struct LogVSOutputHelper {
    LogVSOutputHelper& operator=(const LogVSOutputHelper&) = delete;
    LogVSOutputHelper(const LogVSOutputHelper& other) = delete;
    LogVSOutputHelper(const std::string& log_prefix, const std::string& log_directory, size_t flush_policy = 1);
    ~LogVSOutputHelper();
...
LogVSOutputHelper::LogVSOutputHelper(const std::string& log_prefix, const std::string& log_directory, size_t flush_policy)
...
LogVSOutputHelper::~LogVSOutputHelper() {
    ...
    ss_exit << "\nLogger file shutdown at: " << g3::localtime_formatted(now, g3::internal::time_formatted) << "\n\n";
    filestream() << ss_exit.str() << std::flush;
}

void LogVSOutputHelper::fileWrite(std::string message) {
    //rotateLog();
    fileWriteWithoutRotate(message);
}

void LogVSOutputHelper::fileWriteWithoutRotate(std::string message) {
...
void LogVSOutputHelper::flushPolicy() {
...
void LogVSOutputHelper::setFlushPolicy(size_t flush_policy) {
    flush();
    flush_policy_ = flush_policy;
    flush_policy_counter_ = flush_policy;
}
...
void LogVSOutputHelper::flush() {
    filestream() << std::flush;
}



//std::string LogVSOutputHelper::changeLogFile(const std::string& directory, const std::string& new_name) {
...
* @return
*/
//bool LogVSOutputHelper::rotateLog() {
...
*/
//void LogVSOutputHelper::setLogSizeCounter() {
//    std::ofstream& is(filestream());
...
* @return
*/
//bool LogVSOutputHelper::createCompressedFile(std::string file_name, std::string gzip_file_name) {
...
//        gzwrite(output, buffer, static_cast<unsigned int>(N));
//    }
...
//std::string LogVSOutputHelper::logFileName() {
//    return log_file_with_path_;
//}

void LogVSOutputHelper::addLogFileHeader() {
    filestream() << header();
}
